...
def draw_policies(bot, game):
	...
	game.hiddenhistory.append(("*Tur %d.%d*\nBaşkan %s'ın çektiği yasalar " % (game.board.state.liberal_track + game.board.state.fascist_track + 1, game.board.state.failed_votes + 1, game.board.state.president.name) ) + hiddenhistory_text)
...
def choose_policy(update: Update, context: CallbackContext):
	...
	try:
		...
		if len(game.board.state.drawn_policies) == 3 and uid == game.board.state.president.uid:
			...
			game.hiddenhistory.append("Başkanın elediği kart " + answer)
			for i in range(3):
			...
			if answer == "veto" :
				...
				btns = [[InlineKeyboardButton("Veto! (öneriyi kabul et)", callback_data=strcid + "_yesveto")],
				[InlineKeyboardButton("Veto İptal! (öneriyi reddet)", callback_data=strcid + "_noveto")]]
...
def shuffle_policy_pile(bot, game):
	...
	if len(game.board.policies) < 3:
		game.history.append("*Yasa destesinde yeterli kart kalmadı, ben de kalan kartları çekme destesiyle karıştırdım.!*")
		game.hiddenhistory.append("* Yasa destesinde yeterli kart kalmadı, ben de kalan kartları çekme destesiyle karıştırdım.!*")
...
def main():
	...
	dp.add_handler(CommandHandler("yardim", Commands.command_help))
	dp.add_handler(CommandHandler("tablo", Commands.command_board))
	dp.add_handler(CommandHandler("kurallar", Commands.command_rules))
	dp.add_handler(CommandHandler("heil", Commands.command_ping))
	dp.add_handler(CommandHandler("semboller", Commands.command_symbols))
	dp.add_handler(CommandHandler("stats", Commands.command_stats))
	dp.add_handler(CommandHandler("yenioyun", Commands.command_newgame))
	dp.add_handler(CommandHandler("oyunubaslat", Commands.command_startgame))
	dp.add_handler(CommandHandler("oyuniptal", Commands.command_cancelgame))
	dp.add_handler(CommandHandler("katil", Commands.command_join))
	dp.add_handler(CommandHandler("history", Commands.command_showhistory))
	dp.add_handler(CommandHandler("oylar", Commands.command_votes))
	dp.add_handler(CommandHandler("oyver", Commands.command_calltovote))	
